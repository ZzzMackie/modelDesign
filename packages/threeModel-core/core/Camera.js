...
export class Camera {
  ...
  changeCamera(cameraType) {
    ...
    this.renderer__three.updateRenderer(); // 重新渲染
  }
  updateAspect(aspect) {
    //防止模型变形
  ...
  screenshot(w = 600, h = 600, type = 'image/png', transparentBackground = false, encoderOptions = 1) {
    ...
    resizedContext.drawImage(this.renderer__three.renderer.domElement, 0, 0, w, h);
    if (transparentBackground) {
    ...
    return resizedCanvas.toDataURL(type, encoderOptions);
  }
}
